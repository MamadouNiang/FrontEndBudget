...
export class MouvementsComponent implements OnInit {
  ...
  constructor(private modalService: BsModalService,private service: SaisieService,private router:Router) {
  }
  ...
  getTableMvI(){
    ...
    this.service.getAllMVT().subscribe(data => {
      for(let i =0 ; i< data.length;i++){
        if(!data[i].validaton && !data[i].rejeter){
          val.push(data[i]);
        }
      }
    }, error => {
  ...
  confirm2(data: any): void {
    this.message = 'Confirmed!';
    this.modalRef.hide();
    for(let i=0;i<data.length;i++){
      if(!data[i].rejeter){
        data[i].rejeter = true;
        this.service.saveMvmR(data[i],data[i].matricule).subscribe(data=>{
        },error => {
        })
      }
    }
    this.router.navigateByUrl("/rejeter")
  }
